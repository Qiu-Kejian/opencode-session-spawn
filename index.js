...
/**
 * session-spawn —— 会话生成原语（v2.0.0，2026-09-12 卍解简化定稿）
 *
 * 只做一件事：创建一个新的顶层会话，并把起始语句注入为该会话首条消息。
 *   create: client.session.create({ body: { title } })
 *   inject: client.session.promptAsync({ path: { id }, body: { parts: [{ type: "text", text }] } })
 *
 * 设计约束（用户定稿）：
...
export const SessionSpawn = async (ctx) => {
  ...
  // 核心：建新会话 + 注入起始语句。
  // 返回 { id, title, injected, error? }；create 失败则抛出（未建会话）。
  async function spawn(prompt, title) {
    ...
    const created = await client.session.create({ body: { title: t } });
    ...
    const injectFn =
      typeof client.session.promptAsync === "function"
        ...
        : typeof client.session.prompt === "function"
          ...
          : null;
    if (!injectFn) return { id, title: t, injected: false, error: "client.session.promptAsync/prompt 均不可用" };

    try {
      await injectFn({ path: { id }, body });
      return { id, title: t, injected: true };
    } catch (e) {
      console.error(`[session-spawn] 注入失败（会话 ${id} 已建保留）: ${e.message}`);
      return { id, title: t, injected: false, error: e.message };
    }
  }
  ...
  return {
    ...
    tool: {
      spawn_session: tool({
        description:
          "创建一个新的顶层会话并向其注入起始语句，用于无人值守长任务编排：当前会话模型在需要时自行决定把工作交给一个新会话继续。新会话会立即开始执行该起始语句。若需让其读取某份交接文书，请把文书路径与读取要求写进 prompt。",
        args: {
    ...
    // 人工命令：@spawn <起始语句>（兼容 @relay spawn）。
    "chat.message": async ({ sessionID }, { parts }) => {
      try {
        const text = userText(parts);
        const m = /^\s*@(?:relay\s+)?spawn\b([\s\S]*)$/i.exec(text);
        if (!m) return;
        const prompt = m[1].trim();
        if (!prompt) {
          await notify("spawn", "用法：@spawn <起始语句>", "warning");
          return;
        }
        try {
